...
// we are finding the path using the URL coz it is not inside the src directory and we want to be able to run this script from the root of the project without issues
...
async function uploadSystemVoiceAudio({
  ...
  if (error) {
    throw new Error(
      `Failed to upload system voice \"${key}\" to Supabase Storage: ${error.message}`,
    );
  }
}
...
// this file is meant to be run from the root of the project and uploads the corresponding audio files to Supabase Storage